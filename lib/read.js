...
const matrixread = function(options) {
  ...
  function matrix(vfb) {
    ...
    for (let i=1;i<gtnjtarget;i++) {
      ...
      try {
        let narray = scandatamatrix(i)
        something = rs.decode(narray)
        found = true
        break
      }
      catch(e) {
    ...
    if (protectedbit) {
      ...
      gphx = Array.from(aesjs.padding.pkcs7.strip(gphx))
